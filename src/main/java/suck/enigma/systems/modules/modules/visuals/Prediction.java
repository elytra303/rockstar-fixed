...
import suck.enigma.systems.setting.settings.shared.PredicateValue;
import suck.enigma.utility.colors.ColorRGBA;
import suck.enigma.utility.colors.Colors;
import suck.enigma.utility.game.PotionUtility;
...
public class Prediction extends BaseModule {
   ...
   private final EventListener<Render3DEvent> onRender3D = event -> {
      ...
      float size = 1.0F;
      Identifier id = enigma.id("textures/hit.png");
      RenderSystem.setShaderTexture(0, id);
      RenderSystem.setShader(ShaderProgramKeys.POSITION_TEX_COLOR);
      BufferBuilder buffer = RenderSystem.renderThreadTesselator().begin(DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
