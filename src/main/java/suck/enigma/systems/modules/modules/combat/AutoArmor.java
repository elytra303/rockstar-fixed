...
import suck.enigma.utility.inventory.EnchantmentUtility;
...
public class AutoArmor extends BaseModule {
   ...
   private void evaluateCurrentArmorValues(PlayerInventory inventory, int[] bestArmorSlots, int[] bestArmorValues) {
      ...
      for (int slot = 0; slot < 36; slot++) {
         ...
         if (!stack.isEmpty() && stack.getItem() instanceof ArmorItem item) {
            EquipmentSlot equipmentSlot = ((ArmorItemAddition)item).enigma$getType().getEquipmentSlot();
            int armorType;
   ...
   private int getArmorValue(ArmorItem item, ItemStack stack) {
      ArmorMaterial material = ((ArmorItemAddition)item).enigma$getMaterial();
      EquipmentType equipmentType = ((ArmorItemAddition)item).enigma$getType();
