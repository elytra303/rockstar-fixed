...
import suck.enigma.ui.hud.impl.island.ExtandableStatus;
...
public class MusicStatus extends ExtandableStatus implements IMinecraft {
   ...
   public void draw(CustomDrawContext context) {
      ...
      if (tracker.haveActiveSession() && tracker.getSession() != null) {
         ...
         Identifier trackImage = tracker.getImage() != null ? tracker.getImage() : enigma.id("icons/music/no_image.png");
         ...
         if (extending != 0.0F && tracker.getSession() != null) {
            ...
            if (extending > 0.7F) {
               ...
               context.drawTexture(enigma.id("icons/music/previous.png"), previous, textColor.withAlpha(255.0F - 100.0F * this.hoverPrevious.getValue()));
               ...
               context.drawTexture(
                  enigma.id("icons/music/play.png"), pause, textColor.withAlpha(255.0F * (1.0F - anim) - 100.0F * this.hoverPause.getValue())
               );
               ...
               context.drawTexture(enigma.id("icons/music/pause.png"), pause, textColor.withAlpha(255.0F * anim - 100.0F * this.hoverPause.getValue()));
               RenderUtility.end(context.getMatrices());
               RenderUtility.end(context.getMatrices());
               context.drawTexture(enigma.id("icons/music/next.png"), next, textColor.withAlpha(255.0F - 100.0F * this.hoverNext.getValue()));
            }
            ...
            if (owner != null) {
               context.drawTexture(enigma.id("icons/media/" + owner + ".png"), x + expWidth - 22.0F, y + expHeight - 21.0F, 8.0F, 8.0F, ColorRGBA.WHITE);
            }

            switch (WindowsMediaSession.getCycle()) {
               case 0:
                  context.drawTexture(enigma.id("icons/music/repeat.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor.withAlpha(150.0F));
                  break;
               case 1:
                  context.drawTexture(enigma.id("icons/music/repeat.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor);
                  break;
               case 2:
                  context.drawTexture(enigma.id("icons/music/repeat1.png"), x + 14.0F, y + expHeight - 21.0F, 8.0F, 8.0F, textColor);
            }
            ...
            if (!tracker.getLyrics().isEmpty() && tracker.getLyrics().split("butors\\n\\n").length > 1) {
               ...
               context.drawTexture(enigma.id("icons/music/text.png"), lyricsRect, textColor.withAlpha(255.0F - 100.0F * this.hoverLyrics.getValue()));
            }
