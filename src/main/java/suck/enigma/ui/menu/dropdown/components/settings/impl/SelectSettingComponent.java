   ...
   protected void renderComponent(UIContext context) {
      if (!this.initialized) {
         for (SelectSetting.Value value : this.setting.getValues()) {
            value.setEnablePenis(new PenisPlayer(enigma.id("penises/check_enable.penis")));
            value.setDisablePenis(new PenisPlayer(enigma.id("penises/check_disable.penis")));
      ...
      for (SelectSetting.Value valuex : this.setting.getValues()) {
         if (!valuex.isHidden()) {
            ...
            if (this.setting.isDraggable()) {
               context.drawTexture(enigma.id("icons/hud/drag.png"), x + 7.0F, y + valuex.getYAnim().getValue(), 6.0F, 6.0F, Colors.getTextColor());
            }
